...
import de.mlessmann.hwserver.Main;
import de.mlessmann.hwserver.allocation.GroupMgrSvc;
import de.mlessmann.hwserver.allocation.session.SessionMgrSvc;
import de.mlessmann.hwserver.network.HWTCPServer;
import de.mlessmann.hwserver.reflections.AuthLoader;
import de.mlessmann.hwserver.reflections.AuthProvider;
import de.mlessmann.hwserver.reflections.CommHandProvider;
import de.mlessmann.hwserver.reflections.CommandLoader;
import de.mlessmann.hwserver.tasks.FSCleanTask;
import de.mlessmann.hwserver.tasks.ITask;
import de.mlessmann.hwserver.tasks.TaskManager;
...
public class HWServer implements ILogReceiver, IFutureListener {
     ...
     * @see #getLogFile()
     */
    private final String LOGFILE = "de.mlessmann.hwserver-latest.log";

    /**
     * Logger
     */
    private final MarkL4YGLogger logger = new MarkL4YGLogger("de/mlessmann/hwserver");
    ...
    public HWServer preInitialize() throws IOException {
        ...
        */
        onMessage(this, INFO, "------ ------ Entering preInitialization ------ ------");
    ...
    public HWServer setArg(AppArgument a) {
        ...
        if (a.getValue() != null && !a.getValue().equals("")) {
            ...
            switch (a.getKey()) {
                ...
                case "--log-no-trace": logger.setLogTrace(false); break;
